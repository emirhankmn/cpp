...
ClapTrap::ClapTrap(std::string name) : name(name), hitPoints(10), energyPoints(10), attackDamage(0) {
    std::cout << this->name << " to create." << std::endl;
}
...
void ClapTrap::attack(const std::string& target) {
    if (energyPoints > 0 && hitPoints > 0) {
        ...
        if (hitPoints == 0) {
            std::cout << name << " has no life left." << std::endl;
        } 
        else {
            std::cout << name << " does not have enough energy." << std::endl;
        }
    }
}
...
void ClapTrap::takeDamage(unsigned int amount) {
    ...
    std::cout << name << " " << amount << " took damage! Current heart: " << hitPoints << std::endl;
}
...
void ClapTrap::beRepaired(unsigned int amount) {
    if (energyPoints > 0 && hitPoints > 0) {
        ...
        std::cout << name << " " << amount << " heart recover. Current heart: " << hitPoints << std::endl;
    } else {
        if (hitPoints == 0) {
            std::cout << name << " has no life left." << std::endl;
        } 
